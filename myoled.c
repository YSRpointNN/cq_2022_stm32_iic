...
void OLED_write_data(uint8_t xdata){
	IIC_start();
	IIC_send_byte(0x78);
	IIC_send_byte(0x40);
	IIC_send_byte(xdata);
...
void OledShowChar(uint8_t line, uint8_t col, char dischar){
	uint8_t i;
	OLED_set_cursor((line - 1) * 2, (col - 1) * 8);
	for (i = 0; i < 8; i++){
		OLED_write_data(OLED_F8x16[dischar - ' '][i]);
	}
	OLED_set_cursor((line - 1) * 2 + 1, (col - 1) * 8);
	for (i = 0; i < 8; i++){
		OLED_write_data(OLED_F8x16[dischar - ' '][i + 8]);
	}
}

void OledShowNumb(uint8_t line, uint8_t col, unsigned int numb){
	unsigned int lenbuffer = numb;
    unsigned char len = 0;
    while (lenbuffer){
        lenbuffer /= 10;
        len += 1;
    ...
    for(i=0;i<len;i++){
        ...
        char single_char = (numb % wei ) / (wei / 10) + 0x30;
        OledShowChar(line, col + i, single_char);
    }
}
